...
CEffectList *GetEffectListFromID(DWORD unique_id)
{
	DWORD i;
...
BOOL DeleteParticleFromMagicEffect(DWORD unique_id)	//지정된 매직에서 파티클만 지운다.
{
	for(int i=0; i<stMLcnt; i++ )
	{
		if( stMagicList[i].mUniqueID == unique_id )	//해당되는 엔티티들을 죄다///
		{
			...
			for( int j=0; j<stMagicList[i].mEntityUniqueIDCnt; j++)
			{
				CEffectList *entity = GetEffectListFromID(stMagicList[i].mEntityUniqueID[j]);
				if( entity && entity->mFlag & _MT_FLAG_PARTICLE )//파티클인경우.
					SetDisapearTimeEffect(stMagicList[i].mEntityUniqueID[j],0);
				ret=1;
			}
			return ret;	//엔티티가 전부사라진 경우는 리턴값이 FALSE다.
		}
...
//		ret=((CProcessMagic *)stMagicList[i].mMagicPtr)->GetEntityIDs(stMagicList[i].mOldFrame,stMagicList[i].mFrame,start_frame,&magic_track);
		...
		if( ret >= 0 )	//엔티티가 있다. 뿌리자.
		{
			...
			for(j=start_j; j<ret+start_j; j++)
			{
				...
				{
					EFUtil_PushEffectValue(&stMagicList[i].mEInfo,TRUE);//수환씨 이펙트 큐에 넣는다.
				}
