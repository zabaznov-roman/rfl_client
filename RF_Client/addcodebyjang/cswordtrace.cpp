...
Vector4f x_abcd[총점 갯수]
Vector4f y_abcd[총점 갯수]

	CalcCubicCurve(x_abcd,총점 갯수 -1, x);
	CalcCubicCurve(y_abcd,총점 갯수-1, y);

	
	step = 5;  //3개씩 
	cnt=(총점 갯수-1)*step;
	최종x[cnt] = CalcEvalCubicCurve(x_abcd[0],0);
	최종y[cnt] = CalcEvalCubicCurve(y_abcd[0],0);
	
	cnt--;
	for(int j = 0; j < 총점 갯수-1; j++)
	{
		for(i = 1; i <= step; i++)	//각각의 사이는 스탭값으로.
		{
			float u = (i) / (float)step;
			최종x[cnt]	= CalcEvalCubicCurve(x_abcd[0][j],u);
			최종y[cnt]	= CalcEvalCubicCurve(x_abcd[1][j],u);
			cnt--;
		}
	}
*/
...
//	GetD3dDevice()->LightEnable( 0,FALSE);
    ...
    pd3dDevice->SetRenderState( D3DRS_VERTEXBLEND,     TRUE);
