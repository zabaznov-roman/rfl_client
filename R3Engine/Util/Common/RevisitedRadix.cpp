...
RadixSorter& RadixSorter::Sort(udword* input, udword nb, bool signedvalues)
{
	...
	if(nb>mBufferCurrentSize)
	{
		// Free previously used ram
		RELEASEARRAY_MALLOC(mIndices2);
		RELEASEARRAY_MALLOC(mIndices);

		// Get some fresh one
		mIndices		= (udword *)malloc(sizeof(udword)*nb);
		mIndices2		= (udword *)malloc(sizeof(udword)*nb);

		mBufferCurrentSize = nb;
	...
	// Clear counters
	memset(mHistogram,0, 256*4*sizeof(udword));
