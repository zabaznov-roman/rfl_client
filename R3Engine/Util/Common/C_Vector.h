...
typedef double Vector2d[2];  /* st */
typedef double Vector3d[3];  /* xyz */
typedef double Vector4d[4];  // xyzw 또는 평면 ax+by+cz=d;

#define Vector3fp	float *			/* st */
#define Vector4fp	float *			/* st */
...
typedef char Vector3c[3];		// char형 고정 소수점에 쓸 예정...
typedef unsigned char Vector3uc[3];		// char형 고정 소수점에 쓸 예정...
...
#define Vector3fNeg(a,b) {b[0]=-a[0];b[1]=-a[1];b[2]=-a[2];}
...
#define Vector3fDist(a) (sqrtf (a[0]*a[0] + a[1]*a[1] + a[2]*a[2]) )
...
inline void Normalize(Vector3f v) { double a= sqrt((double)v[0]*(double)v[0]+(double)v[1]*(double)v[1]+(double)v[2]*(double)v[2]);
