...
{	
	...
	for (t=start; t<=end; t+=delta) {
		...
		Point3 pos = ap.t;
		Point3 n_pos;
		//-------쓰벌 맥스가 같은거 제대로 못하는 바람에 ..다쉬...
		if( t+delta <= end )
		{
			Matrix3 n_tm = node->GetNodeTM(t+delta) * Inverse(node->GetParentTM(t+delta));
			decomp_affine(n_tm, &ap);
			n_pos = ap.t;
		}
		else
		{	//개 값으로 틀리게하자.
		...
		if (t!= start && EqualPoint3(pos, prevPos) && EqualPoint3(pos, n_pos)) {
			// Skip identical keys 
			continue;
		}
...
{	
	...
	{
		//--------------------
//		if (os.obj && os.obj->SuperClassID()==CAMERA_CLASS_ID)	//카메라일때
//			tm = node->GetObjTMAfterWSM(t) * Inverse(node->GetParentTM(t));
//		else
			tm = node->GetNodeTM(t) * Inverse(node->GetParentTM(t));
...
//		prevQ = ap.q;
		...
		if( b_q.x == q.x &&  b_q.y == q.y &&  b_q.z == q.z && b_q.w == q.w 
		&& a_q.x == q.x &&  a_q.y == q.y &&  a_q.z == q.z && a_q.w == q.w )
			continue;
		b_q=q;
/*		//이러면 문제가 있다...
		if (q.IsIdentity() && t !=0 && t!= end) {
