...
static INode* stNowNode;
static BOOL stIsCameraObj=FALSE;
...
void AsciiExp::ExportLightSettings(LightState* ls, GenLight* light, TimeValue t, int indentLevel)
{
	...
	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_SHADOW_COLOR, Format(light->GetShadColor(t, FOREVER)));
	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_SHADOW_DENSITY, Format(light->GetShadMult(t, FOREVER)));
...
//	Matrix3 ob_tm = node->GetObjectTM(GetStaticFrame());
//	Matrix3 ob_bwsm = node->GetObjTMBeforeWSM(GetStaticFrame());
//	DumpMatrix3(&ob_tm, indentLevel+2);
//	DumpMatrix3(&ob_bwsm, indentLevel+2);
	if( stIsCameraObj )	//카메라일때 문제가 있을때가있다...그래서...
	{
		Matrix3 ob_awtm = node->GetObjTMAfterWSM(GetStaticFrame());
		DumpMatrix3(&ob_awtm, indentLevel+2);
	}
	else
...
TriObject* AsciiExp::GetTriObjectFromNode(INode *node, TimeValue t, int &deleteIt)
{
	...
	if (obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) //삼각 오브젝트로 출력하는군.
	{ 
...
Object* AsciiExp::GetPolyObjectFromNode(INode *node, TimeValue t, int &deleteIt)
{
	deleteIt = FALSE;
	Object *obj = node->EvalWorldState(t).obj;
	...
	if (obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) //삼각 오브젝트로 출력되는것을 리턴.
		return obj;
	else
		return NULL;
	/*
	if (obj->CanConvertToType(Class_ID(POLYOBJ_CLASS_ID, 0))) //삼각 오브젝트로 출력하는군.
	{ 
		PolyObject *poly = (PolyObject *) obj->ConvertToType(t, 
...
void AsciiExp::DumpMatrix3(Matrix3* m, int indentLevel)
{
	...
	Matrix3 tm = stNowNode->GetNodeTM(start) * Inverse(stNowNode->GetParentTM(start));
