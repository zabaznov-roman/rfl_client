...
//#define _NOT_PURE_DEVICE	//Pure 디바이스를 빼려면..

//D3DApp에서 빠지는 부분은 타이머루틴부분..
//그리고 PrePareRender와 EndLoop가 추가된다.
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
	...
	m_bShowCursorWhenFullscreen = GetR3State()->mMouseAccelation;	//마우스 가속.
...
HRESULT CD3DApplication::BuildDeviceList()
{
    ...
    for( UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++ )
    {
        ...
        for( UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++ )
        {
			...
			if( pDevice->DeviceType == D3DDEVTYPE_REF )	//ref는 제외시킨다.
			{
				continue;
			}
    ...
    D3DFORMAT TargetFormat, D3DFORMAT* pDepthStencilFormat )
{
	...
	if( m_dwMinDepthBits == 24 && is_check == FALSE )
	{
		m_dwMinDepthBits = 16;
		is_check=TRUE;
		BOOL b_temp = FindDepthStencilFormat(  iAdapter,  DeviceType, TargetFormat, pDepthStencilFormat );
		is_check=FALSE;
		return b_temp;
	}
...
if( m_bIsFullScreen )
{
	pDeviceInfo->bWindowed=0;
	m_bWindowed=0;
	for(DWORD i=0; i<pDeviceInfo->dwNumModes; i++)
	{
		if( pDeviceInfo->modes[i].Width == m_dwCreationWidth 
			&& pDeviceInfo->modes[i].Height == m_dwCreationHeight )
		{
...
*/
            _tcscpy( strMsg, _T("Direct 3D 8.1이 설치되지 않았습니다.\n")
                             _T("시스템에 DirectX 8.1이상을 설치하십시요\n"));
            break;

        case D3DAPPERR_NOCOMPATIBLEDEVICES:
            _tcscpy( strMsg, _T("Direct3D를 호환하는 디바이스가 없습니다.\n")
...
HRESULT CD3DApplication::PrepareLoop()	// 내부 디바이스의 준비...
{
	HRESULT hr;
...
HRESULT CD3DApplication::EndLoop()	//화면 플리핑이 있다.
{
    m_pd3dDevice->Present( NULL, NULL, NULL, NULL );
