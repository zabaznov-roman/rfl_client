...
//-----------------------------------------------------------------------------
#ifndef __R3D3DAPP_H__
#define __R3D3DAPP_H__
...
protected:
	...
	BOOL			  m_bIsFullScreen; //풀스크린을 쓸건가?
...
public:
    ...
    virtual HRESULT Release(){	Cleanup3DEnvironment();	return S_OK;}; //3D완전 제거.
    virtual INT     Run();
    virtual LRESULT MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam );
    virtual VOID    Pause( BOOL bPause );

	virtual HRESULT PrepareLoop();	// 내부 디바이스의 준비...
	virtual HRESULT EndLoop();	//화면 플리핑이 있다.
...
public:
    // Functions to create, run, pause, and clean up the application
    virtual HRESULT Create( HINSTANCE hInstance );	//3D환경시작.
    virtual HRESULT Release(){	Cleanup3DEnvironment();	return S_OK;}; //3D완전 제거.
    virtual INT     Run();
    virtual LRESULT MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam );
    virtual VOID    Pause( BOOL bPause );
