...
BYTE * JpegFileToRGB(char* fileName,
							   UINT *width,
							   UINT *height)
...
BOOL GetJPGDimensions(char* fileName,
								UINT *width,
								UINT *height)
...
BYTE *RGBFromDWORDAligned(BYTE *inBuf,
									UINT widthPix, 
									UINT widthBytes,
									UINT height)
{
...
BOOL ARGBToJpegFile(char* fileName, 
							BYTE *dataBuf,
							UINT pitch,						// 길이다 
							UINT widthPix,
							UINT height,
...
//			AfxMessageBox("Memory error");
		...
		for (row=0;row<height;row++) {
			for (col=0;col<widthPix;col++) {
				LPBYTE pRed, pGrn, pBlu;
				pRed = dataBuf + row * pitch + col * 4;
				pGrn = dataBuf + row * pitch + col * 4 + 1;
				pBlu = dataBuf + row * pitch + col * 4 + 2; // 
	...
	{
		tmp = (BYTE*)new BYTE[widthPix*height*3];
		if (tmp==NULL) {
			return FALSE;
		}
...
//				int lum = (int)(.299 * (double)(*pRed) + .587 * (double)(*pGrn) + .114 * (double)(*pBlu));
				LPBYTE pGray;
				pGray = tmp + row * widthPix*3 + col*3;
				pGray[2] = *pRed;
				pGray[1] = *pGrn;
				pGray[0] = *pBlu;
			}
...
/* Now use the library's routine to set default compression parameters.
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
	...
	if (color) {
//		outRow = dataBuf + (cinfo.next_scanline * widthPix * 3);^
		outRow = tmp + (cinfo.next_scanline * widthPix * 3);
	} else {
		outRow = tmp + (cinfo.next_scanline * widthPix);
	}
