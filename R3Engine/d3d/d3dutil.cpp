...
{
    LPD3DXBUFFER pCode;
    LPD3DXBUFFER pConstants;
    LPD3DXBUFFER pErrCode;
 //   TCHAR        strPath[MAX_PATH];
    HRESULT      hr;
...
//    DXUtil_FindMediaFile( strPath, strFilename );
    ...
    if( FAILED( hr = D3DXAssembleShaderFromFile( strFilename, 0, &pConstants, &pCode, &pErrCode ) ) )
	{
...
HRESULT D3DUtil_SetDeviceCursor( LPDIRECT3DDEVICE8 pd3dDevice, DWORD dwWidth, DWORD dwHeight, LPDIRECT3DTEXTURE8 pSrcTex )
{
	...
	// Create a surface for the fullscreen cursor
    if( FAILED( hResult = pd3dDevice->CreateImageSurface( dwWidth, dwHeight,
        D3DFMT_A8R8G8B8, &pCursorSurface ) ) )
    {
	...
	// Transfer cursor image into the surface
    D3DLOCKED_RECT dest_rect, src_rect;

	pCursorSurface->LockRect( &dest_rect, NULL, 0 );		
	pSrcTex->LockRect( 0, &src_rect, NULL, 0 );

	for( WORD y=0; y<dwHeight; ++y )
	{
		memcpy( &((BYTE *)dest_rect.pBits)[y*dest_rect.Pitch], 
				&((BYTE*)src_rect.pBits)[y*src_rect.Pitch], src_rect.Pitch );
	}
    ...
    // Set the device cursor
    if( FAILED( hResult = pd3dDevice->SetCursorProperties( 0, 0, pCursorSurface ) ) )
    {
		SAFE_RELEASE( pCursorSurface );
        return -1;
    }	
